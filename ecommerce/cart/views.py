...
def cart_view(request):
        ...
        for i in c:
            total += i.quantity*i.product.price    #Calculates the sum of each product price8quantity
...
def payment_status(request,u):
    ...
    if(request.method=="POST"):
        ...
        try:
            ...
            #To reteieve a particular record in Payment Table whose order id matches the response order id
            p=Payment.objects.get(order_id=response['razorpay_order_id'])
            p.razorpay_payment_id=response['razorpay_payment_id']  #adds payment id after successful payments
